...
__crow_build_completion_request() {
...
__crow_bash_autocomplete() {
  ...
  if [[ "${words[0]}" != "source" ]]; then
    ...
    __crow_init_completion -n "=:" || return

    request_comp="$(__crow_build_completion_request)"
    ...
    if [[ "${COMP_TYPE:-}" == "63" && ${#matches[@]} -gt 0 ]]; then
      ...
      for candidate in "${matches[@]}"; do
        local desc="$(__crow_find_description_for "${candidate}")"
...
complete -o bashdefault -o default -F __crow_bash_autocomplete crow
